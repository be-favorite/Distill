...
5월은 재직 중인 연구센터에서 맡고 있는 메인 연구과제 \<빅데이터를 이용한 미세먼지 건강영향평가\>에서 메타분석을 수행하는데에 가장 많은 시간을 할애했습니다. 5월 뿐만이 아니라 실무에서 가장 많은 시간을 할애한 과제이기도 합니다. 작년부터 수행해 온 해당 과제에 관한 분석이 얼추 모두 마무리가 되어갑니다. 대학원을 졸업하고 실무에 나와 처음으로 맡은 프로젝트이기도 했고, 데이터 수집, 데이터 마트 구축 부터 분석, 보고서 작성까지 처음부터 끝까지 맡아 진행한 과제였어서 그런지 정도 많이 가지만, 데이터 분석의 end-to-end를 모두 혼자 처리했어야 했기에... 솔직히 이제 그만하고 싶은 마음도 듭니다. 가장 중요한 논문 작업 단계가 남아있는데, 이런 마음이 드는게 정상인지 모르겠네요.😂 그만큼 정말 많은 양의 일을 저 혼자 처리했어야하기도 했고.. 중간중간에 여러 난관에 봉착하기도 했으며 그것을 해결해나면서 쾌감을 느끼기도 했습니다. 이번 월간 회고에서는 제가 겪은 크고 작은 난관들에 대해 이야기해보려고 합니다.

## 1 SAS를 다시 손에 잡다

본 연구과제는 건강보험공단 빅데이터 맞춤형 연구DB로 진행됩니다. 자료의 특성 상 데이터 프라이버시때문에 건강보험공단에서 운영 중인 분석센터에 직접 가야지만 데이터를 열람하고 처리할 수 있습니다. 연구DB 신청 당시 분석 툴에 SAS + R을 선택할 수 있길래 당연히 데이터를 확인하고 처리하는 것도 R로 가능할 것이라 생각 했는데, 구축된 데이터는 SAS 서버 상에 놓여져있더군요. 데이터 특성상 보통 수십기가는 넘기때문에 분석센터의 로컬컴퓨터에서 제공되는 R로는 구축한 원 자료를 핸들링하는 것은 불가능합니다..즉, SAS로 원 자료를 랭글링[^1]해야만 하는 상황이였습니다.
...
본 과제 이름은 \<빅데이터를 이용한 미세먼지 건강영향평가\>이긴 하나, 6종의 대기오염원(${\rm{PM}}_{10}$, ${\rm{PM}}_{2.5}$, ${\rm{NO}}_{2}$, ${\rm{SO}}_{2}$, ${\rm{O}}_{3}$, ${\rm{CO}}$)에 관해 모두 평가를 실시했어야 했습니다. 그래서, 이 "건강영향평가"는 어떻게 실시하냐? 질병의 일 발생/입원 건수를 $Y$로 잡고, 일 대기오염원 농도를 $X$로 잡아서 시계열 회귀모델링을 실시하는 것이죠. 대기오염원들이 특정 질병에 미치는 건강영향을 평가하는 것입니다. 여기서, 대기오염원이 미치는 효과는 비선형(non-linear)의 형태를 띠고, 지연 효과(lag effect)[^3]까지 고려해주어야 했죠. Distributed lag non-linear model[@gasparrini2010] (이하, DLNM)은 둘을 동시에(simultaneously) 모델링 하게끔 해줍니다. 환경적 요인들로 Outcome을 모델링하는 생태학적 연구분야에서는 DLNM이 최신 시계열 회귀모형이라고 보시면 될 것 같습니다. 본 분야의 모델링 쪽은 Gasparrini 교수가 주도하고 있구요.

[^3]: 예를 들자면, 5일 전에 마신 고농도의 초미세먼지가 질병 발생에 영향을 미치는 것

대학원에서 가장 많이 시간을 할애했던 연구 분야는 시계열 자료분석이었는데, 다행히 이때 시계열을 회귀적으로 모델링하는 접근과 Distributed lag model에 대해 공부했던 경험이 있어 DLNM으로의 확장은 크게 어렵지 않았습니다.😀
